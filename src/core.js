...
const updateIntroLine = (index, statusText = 'OK') => {
  if (introLines[index]) {
    ...
    introLines[index].textContent = `${currentText}${introLines[
      index
    ].textContent.slice(
      introLines[index].textContent.indexOf('.')
    )}[${statusText}]`
...
const initializeApplication = async () => {
  try {
    ...
    //separate try/catch so it doesn't crash the whole app
    try {
      await soundModule.soundDiagnostics()
      updateIntroLine(4)
    } catch (error) {
      console.warn('[SOUND] Diagnostics failed:', error)
      updateIntroLine(4, 'FAIL')
    }
    ...
    introLines.forEach(line => {
      if (!line.textContent.includes('[')) {
        ...
        line.textContent = `${currentText}${line.textContent.slice(
