...
// --- NAVIGATION & 3D CONTROL ---



function rotateTo(mode) {
    const cube = document.getElementById('cube');
    const navEnc = document.getElementById('nav-encrypt');
    const navDec = document.getElementById('nav-decrypt');

    if (mode === 'decrypt') {
        cube.classList.add('show-decrypt');
        navDec.classList.add('active');
        navEnc.classList.remove('active');
    } else {
        cube.classList.remove('show-decrypt');
        navEnc.classList.add('active');
        navDec.classList.remove('active');
    }
...
// Fixed strength meter to find the label/bar relative to the input being typed in
function updateStrength(inputEl) {
    const pass = inputEl.value;
    const parent = inputEl.closest('.panel-content');
    const bar = parent.querySelector('.strength-bar');
...
async function copyToClipboard(btn) {
    const parent = btn.closest('.pane');
    const text = parent.querySelector('.resultOutput').innerText;
